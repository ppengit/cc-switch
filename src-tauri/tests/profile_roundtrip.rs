...
#[test]
fn profile_switch_aborts_when_takeover_restore_fails() {
    let _guard = test_mutex().lock().expect("acquire test mutex");
    reset_test_fs();
    let _home = ensure_test_home();
    ...
    state
        .db
        .set_current_provider(AppType::Claude.as_str(), "custom1")
        .expect("set current custom1");
    futures::executor::block_on(async {
        state
            .db
            .save_live_backup("claude", "{invalid-json")
            .await
            .expect("seed malformed takeover backup");
        ...
        state
            ...
            .await
            .expect("enable claude takeover flag");
    });
    ...
    payload.providers.claude = Some("custom2".to_string());
    project.payload = serde_json::to_string(&payload).expect("serialize payload");
    state.db.save_profile(&project).expect("save project");

    let error = ProfileService::apply(&state, &project.id, ProfileScope::Claude)
        .expect_err("malformed takeover backup must abort profile apply");
    assert!(error.to_string().contains("auto-disable proxy takeover"));
    assert_eq!(
        state
            .db
            .get_current_provider(AppType::Claude.as_str())
            ...
            .as_deref(),
        Some("custom1"),
        "failed restore must not switch providers"
    );
    assert_eq!(
        state
            .db
        ...
        None,
        "failed restore must not mark the target profile as current"
    );
    let (enabled, _) = state.db.get_proxy_flags_sync("claude");
    assert!(
        enabled,
        "failed restore must retain takeover state for recovery"
    );
    assert!(
        futures::executor::block_on(state.db.get_live_backup("claude"))
            .expect("read backup")
            .is_some(),
        "failed restore must retain the only recovery backup"
    );
}
