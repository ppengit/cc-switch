...
impl Database {
    ...
    pub async fn get_proxy_config_for_app(
        ...
        let result = {
            let conn = lock_conn!(self.conn);
            conn.query_row(
                "SELECT app_type, enabled, force_model_enabled, force_model, auto_failover_enabled,
                ...
                |row| {
                    Ok(AppProxyConfig {
                        ...
                        force_model_enabled: row.get::<_, i32>(2)? != 0,
                        force_model: row.get(3)?,
    ...
    pub async fn update_proxy_config_for_app(
        ...
        conn.execute(
            "UPDATE proxy_config SET
                enabled = ?2,
                force_model_enabled = ?3,
                force_model = ?4,
                auto_failover_enabled = ?5,
                max_retries = ?6,
            ...
            rusqlite::params![
                ...
                if config.force_model_enabled { 1 } else { 0 },
                config.force_model,
