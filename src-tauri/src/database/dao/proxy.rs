...
impl Database {
    ...
    pub async fn get_proxy_config_for_app(
        ...
        let result = {
            let conn = lock_conn!(self.conn);
            conn.query_row(
                "SELECT app_type, enabled, auto_failover_enabled, load_balancing_enabled, load_balancing_sticky_minutes,
                        response_rescue_enabled, response_rescue_empty_2xx_enabled,
                        response_rescue_429_enabled, response_rescue_max_retries,
                ...
                |row| {
                    Ok(AppProxyConfig {
                        ...
                        load_balancing_sticky_minutes: row.get::<_, i32>(4)?.max(0) as u32,
                        response_rescue_enabled: row.get::<_, i32>(5)? != 0,
                        response_rescue_empty_2xx_enabled: row.get::<_, i32>(6)? != 0,
                        response_rescue_429_enabled: row.get::<_, i32>(7)? != 0,
                        response_rescue_max_retries: row.get::<_, i32>(8)?.max(0) as u32,
                        max_retries: row.get::<_, i32>(9)? as u32,
                        streaming_first_byte_timeout: row.get::<_, i32>(10)? as u32,
                        streaming_idle_timeout: row.get::<_, i32>(11)? as u32,
                        non_streaming_timeout: row.get::<_, i32>(12)? as u32,
                        circuit_failure_threshold: row.get::<_, i32>(13)? as u32,
                        circuit_success_threshold: row.get::<_, i32>(14)? as u32,
                        circuit_timeout_seconds: row.get::<_, i32>(15)? as u32,
                        circuit_error_rate_threshold: row.get(16)?,
                        circuit_min_requests: row.get::<_, i32>(17)? as u32,
                    })
                },
            )
        };
        ...
        match result {
            ...
            Err(rusqlite::Error::QueryReturnedNoRows) => {
                ...
                Ok(AppProxyConfig {
                    ...
                    response_rescue_enabled: true,
                    response_rescue_empty_2xx_enabled: false,
                    response_rescue_429_enabled: true,
    ...
    pub async fn update_proxy_config_for_app(
        ...
        conn.execute(
            "UPDATE proxy_config SET
                ...
                load_balancing_sticky_minutes = ?5,
                response_rescue_enabled = ?6,
                response_rescue_empty_2xx_enabled = ?7,
                response_rescue_429_enabled = ?8,
                response_rescue_max_retries = ?9,
                max_retries = ?10,
                streaming_first_byte_timeout = ?11,
                streaming_idle_timeout = ?12,
                non_streaming_timeout = ?13,
                circuit_failure_threshold = ?14,
                circuit_success_threshold = ?15,
                circuit_timeout_seconds = ?16,
                circuit_error_rate_threshold = ?17,
                circuit_min_requests = ?18,
                updated_at = datetime('now')
             WHERE app_type = ?1",
            rusqlite::params![
                config.app_type,
                ...
                config.load_balancing_sticky_minutes as i32,
                if config.response_rescue_enabled { 1 } else { 0 },
                if config.response_rescue_empty_2xx_enabled {
                    1
                } else {
                    0
                },
                if config.response_rescue_429_enabled {
                    1
                } else {
                    0
                },
                config.response_rescue_max_retries.min(10) as i32,
...
mod tests {
    ...
    #[tokio::test]
    async fn update_proxy_config_for_app_keeps_load_balancing_when_prerequisites_are_met(
    ) -> Result<(), AppError> {
        let db = Database::memory()?;
        let config = AppProxyConfig {
            app_type: "claude".to_string(),
        ...
        assert!(
            updated.load_balancing_enabled,
            "load balancing should persist when takeover + auto failover are enabled"
        );
