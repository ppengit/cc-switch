...
- **Codex Request Model Preservation**: When request-model alias mapping is disabled or has no matching route, the model selected by Codex CLI is preserved instead of falling back to a provider `ANTHROPIC_MODEL` or `config.toml` default. Saved route data remains intact while disabled, local catalog routing obeys its own switch, and Chat providers use their configured model only when the request omits `model`.
...
- **Controlled Live Configuration Ownership**: Direct writes retain an explicit Live owner, takeover writes remain proxy access templates, Profile switching aborts if takeover restoration fails, and restored backups rebind ownership before later backfill. Codex MCP reconciliation preserves only unmanaged external entries, so DB-known disabled or deleted MCP servers are not resurrected from old backups.
- **Runtime Request Visibility**: Provider rows show real in-flight activity even if the provider was disabled or switched while a request was still running. Real-time activity also supersedes stale mount-time status queries, so ongoing upstream traffic is not hidden by configuration state.

### Changed

- **Upstream v3.16.5 Integration**: Integrated the upstream profile management, usage-dashboard refresh settings, provider presets, Codex catalog/history improvements, session UI updates, documentation, and release infrastructure without replacing the existing customized configuration-file lifecycle and safety rules.
